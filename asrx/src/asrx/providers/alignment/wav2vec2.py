...
SAMPLE_RATE = 16000
def interpolate_nans(x, method="nearest"): return x.interpolate(method=method, limit_direction="both")
PUNKT_LANGUAGES={"en":"english","fr":"french"}
SingleSegment=dict; SingleAlignedSegment=dict; SingleWordSegment=dict; SegmentData=dict; ProgressCallback=callable; AlignedTranscriptionResult=dict
...
class Wav2Vec2Alignment(AlignmentProvider):
    ...
    def align(self, text_segments, audio):
        result = align(text_segments, self.model, self.metadata, audio, self.device)
        return result
