...
class Vad:
    @staticmethod
    def merge_chunks(segments_list, chunk_size, onset, offset):
...
class PyannoteVAD(VADProvider):
    ...
    def detect(self, audio):
        audio_tensor = self.model.preprocess_audio(audio)
        segments = self.model({'waveform': audio_tensor, 'sample_rate': 16000})
        # Format into a list of dicts
        return [{'start': seg.start, 'end': seg.end} for seg in segments]
