...
SAMPLE_RATE = 16000
SingleWordSegment=dict; SingleSegment=dict; ProgressCallback=callable
...
class DiarizationPipeline:
    ...
    def __call__(
        ...
        if isinstance(audio, str):
            import torchaudio
            loaded_audio, sr = torchaudio.load(audio)
            if sr != SAMPLE_RATE:
                loaded_audio = torchaudio.functional.resample(loaded_audio, sr, SAMPLE_RATE)
            if loaded_audio.ndim == 1:
                loaded_audio = loaded_audio.unsqueeze(0)
            elif loaded_audio.ndim > 1 and loaded_audio.shape[0] > 1:
                loaded_audio = loaded_audio.mean(dim=0, keepdim=True)
...
class PyannoteDiarization(DiarizationProvider):
    ...
    def diarize(self, audio, word_segments=None):
        diarize_df = self.pipeline(audio)
        if word_segments:
            transcript_result = {"word_segments": word_segments}
            assigned = assign_word_speakers(diarize_df, transcript_result)
            return {"word_segments": assigned["word_segments"]}
        return diarize_df
